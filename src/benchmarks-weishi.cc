...
template<class Queue>
void throughput_consumer_impl(unsigned producer_num, Queue* queue, sum_t* consumer_sum, std::atomic<unsigned>* last_consumer, cycles_t* t1, bool last) {
    sum_t sum = 0;

    region_guard_t<Queue> guard;
    ConsumerOf<Queue> consumer{*queue};
    unsigned thread_stop_num = producer_num;
	
    for(;;) {
//	bool empty = (*queue).was_empty();
	...
	if(!empty) {
		...
		std::unique_lock<std::mutex> lck(mtx);
		unsigned stopnum = stop ;
		lck.unlock();
		bool breakflag;
		if (last)
			breakflag = stopnum >= producer_num;
		else
			breakflag = stopnum >= producer_num -1;
		if(breakflag){
           		break;
		}
	}
...
template<class Queue>
void throughput_consumer(unsigned producer_num, Queue* queue, sum_t* consumer_sum, std::atomic<unsigned>* last_consumer, cycles_t* t1, Barrier* barrier, bool last) {
    barrier->wait();
    throughput_consumer_impl(producer_num, queue, consumer_sum, last_consumer, t1,last);
}

template<class Queue>
cycles_t benchmark_throughput(HugePages& hp, std::vector<unsigned> const& hw_thread_ids, unsigned M,unsigned producer_thread_count,unsigned consumer_thread_count, bool alternative_placement, sum_t* consumer_sums) {
    set_thread_affinity(hw_thread_ids[producer_thread_count + consumer_thread_count - 1]); // Use this thread for the last consumer.
    unsigned cpu_idx = 0;
	unsigned const N = M / producer_thread_count;

    auto queue = hp.create_unique_ptr<Queue>(ContextOf<Queue>{producer_thread_count, consumer_thread_count});
    std::atomic<cycles_t> t0{0};
        sum_t const expected_sum_M = (M + 1) / 2. * M / producer_thread_count;
    cycles_t t1 = 0;
    std::atomic<unsigned> last_consumer{consumer_thread_count};

    Barrier barrier;
    std::vector<std::thread> threads(producer_thread_count + consumer_thread_count- 1);
    if(alternative_placement) {
        for(unsigned i = 0; i < producer_thread_count; ++i) {
            set_default_thread_affinity(hw_thread_ids[cpu_idx++]);
            threads[i] = std::thread(throughput_producer<Queue>, N, queue.get(), &t0, &barrier);
	}
        for(unsigned i = 0; i < consumer_thread_count; ++i) {
            if(i != consumer_thread_count - 1) { // This thread is the last consumer.
                set_default_thread_affinity(hw_thread_ids[cpu_idx++]);
                threads[producer_thread_count + i] = std::thread(throughput_consumer<Queue>, producer_thread_count, queue.get(), consumer_sums + i, &last_consumer, &t1, &barrier, false);
            }
        }
    } else {
        for(unsigned i = 0; i < producer_thread_count; ++i) {
        ...
        for(unsigned i = 0; i < consumer_thread_count - 1; ++i) {
            set_default_thread_affinity(hw_thread_ids[cpu_idx++]);
            threads[producer_thread_count + i] = std::thread(throughput_consumer<Queue>, producer_thread_count, queue.get(), consumer_sums + i, &last_consumer, &t1, &barrier, false);
        }
    }

    barrier.release(producer_thread_count + consumer_thread_count - 1);
    throughput_consumer_impl(producer_thread_count, queue.get(), consumer_sums + (consumer_thread_count - 1), &last_consumer, &t1,true); // Use this thread for the last consumer.
...
template<class Queue>
void run_throughput_benchmark(char const* name, HugePages& hp, std::vector<unsigned> const& hw_thread_ids, unsigned M, unsigned producer_thread_num,
                              unsigned consumer_thread_num) {
    //int constexpr RUNS = 3;
    int constexpr RUNS = 1;
    std::vector<sum_t> consumer_sums(consumer_thread_num);
//   for(unsigned threads = 1; threads <= consumer_thread_num; ++threads) {
    unsigned threads = consumer_thread_num;
        unsigned const N = M / producer_thread_num;
        ...
        for(bool alternative_placement : {false, true}) {
            ...
            for(unsigned run = RUNS; run--;) {
      		stop =0;
                cycles_t time = benchmark_throughput<Queue>(hp, hw_thread_ids, M, producer_thread_num, threads, alternative_placement, consumer_sums.data());
                ...
                if(auto diff = total_sum - expected_sum * producer_thread_num)
...
void run_throughput_mpmc_benchmark(char const* name, HugePages& hp, std::vector<unsigned> const& hw_thread_ids, Type<Queue>, unsigned thread_count_min = 1) {
    unsigned const producer_thread_count = hw_thread_ids.size() / 2;
    unsigned consumer_thread_count = 1;
	if (thread_count_min > consumer_thread_count)
		consumer_thread_count = thread_count_min;
            std::printf("producer num: %2u,consumer num %2u \n", producer_thread_count, consumer_thread_count);
    run_throughput_benchmark<Queue>(name, hp, hw_thread_ids, 1000000, producer_thread_count, consumer_thread_count);
}
